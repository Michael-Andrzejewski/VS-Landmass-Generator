...
| Key | Meaning |
| --- | --- |
| `rock` | Rock type for this area. The stone, its sand, and its ores all follow it (`granite`, `slate`, `basalt`, ...). |
...
Height comes from a distance-to-coast field, so interiors rise and shores taper naturally, and the coastline is jittered with noise so the grid never shows as stair-steps. The `ideal_island` shape ships as a worked example, generated by `tools/gen_ideal_island.py` from a hand-drawn map.
