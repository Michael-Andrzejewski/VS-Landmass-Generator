...
| `fertility` | Soil richness for grass areas: `verylow`, `low`, `medium`, `high`. |
...
| `pond` | Makes the region a pond this many blocks deep: a water bowl sitting in the surrounding land. |
| `ores`, `forest`, `trees` | As above, but per region. |
| `tree <char> <type> <size>` | A landmark tree at that spot on the map. |

Height comes from a distance-to-coast field, so interiors rise and shores taper naturally, and the coastline is jittered with noise so the grid never shows as stair-steps. Two worked examples ship, each generated by a script in `tools/` from a hand-drawn map: `ideal_island` (a varied island with beach, plains, forest and a rocky slate arm) and `starter_island` (a gentle grassy player-start island, ~150 across, with a white-sand beach, a central pond, slate and peridotite below, and a giant oak):

    /genisland shape=starter_island diameter=150 height=22
